  ...
  return (
    <PageContentBlock title={"SSH Keys"}>
      ...
      <div css={tw`md:flex flex-nowrap my-10`}>
        <ContentBox title={"เพิ่ม SSH Key"} css={tw`flex-none w-full md:w-1/2`}>
          <CreateSSHKeyForm />
        </ContentBox>
        <ContentBox title={"SSH Keys"} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
          <SpinnerOverlay visible={!data && isValidating} />
          {!data || !data.length ? (
            <p css={tw`text-center text-sm`}>{!data ? "กำลังโหลด..." : "ไม่พบ SSH Keys สำหรับบัญชีนี้"}</p>
          ) : (
            data.map((key, index) => (
              <GreyRowBox
                key={key.fingerprint}
                css={[tw`bg-[#060606] flex space-x-4 items-center`, index > 0 && tw`mt-2`]}
              >
