  ...
  return (
    <Formik
      ...
      validationSchema={object().shape({
        email: string()
          .max(191, "ที่อยู่อีเมล (email addresses) ต้องไม่เกิน 191 ตัวอักษร")
          .email("ต้องระบุที่อยู่อีเมลที่ถูกต้องครบถ้วนเพื่อทำการต่อไป")
          .required("ต้องระบุที่อยู่อีเมลที่ถูกต้องครบถ้วนเพื่อทำการต่อไป"),
      ...
      <Form>
        <div css={tw`flex justify-between`}>
          <h2 css={tw`text-2xl`} ref={ref}>
            {subuser
              ? `${canEditUser ? "การแก้ไข" : "ดู"} สิทธิ์ในการเข้าถึงสำหรับ ${subuser.email}`
              : "สร้างผู้ใช้ย่อยใหม่"}
          </h2>
          <div>
        ...
        {!subuser && (
          <div css={tw`mt-6`}>
            <Field
              name={"email"}
              label={"อีเมลผู้ใช้"}
              description={"กรุณากรอกที่อยู่อีเมลของผู้ใช้ที่คุณต้องการเชิญให้เป็นผู้ใช้ย่อยสำหรับเซิร์ฟเวอร์นี้"}
            />
          </div>
        )}
        <div css={tw`my-6`}>
