...
const schema = object().shape({
  directoryName: string().required("โปรดระบุชื่อ directory ที่ถูกต้อง"),
});
    ...
    <Formik onSubmit={submit} validationSchema={schema} initialValues={{ directoryName: "" }}>
      {({ submitForm, values }) => (
        <>
          ...
          <Form css={tw`m-0`}>
            <Field autoFocus id={"directoryName"} name={"directoryName"} label={"ชื่อ"} />
            <p css={tw`mt-2 text-sm md:text-base break-all`}>
              <span css={tw`text-neutral-200`}>directory นี้จะถูกสร้างเป็น&nbsp;</span>
              <Code>
                /home/container/
