            ...
            <div css={[tw`flex flex-wrap`, useMoveTerminology ? tw`items-center` : tw`items-end`]}>
              <div css={tw`w-full sm:flex-1 sm:mr-4`}>
                <Field
                  ...
                  description={
                    useMoveTerminology
                      ? "โปรดป้อนชื่อและตำแหน่งของไฟล์หรือโฟลเดอร์ใหม่ โดยใช้รูปแบบที่เกิดขึ้นในระบบไฟล์แบบเส้นทางสัมพันธ์ (relative path) ต่อจากโฟลเดอร์ปัจจุบัน"
                      : undefined
                  }
                  autoFocus
                />
              ...
              <div css={tw`w-full sm:w-auto mt-4 sm:mt-0`}>
                <Button css={tw`w-full`}>{useMoveTerminology ? "ย้าย" : "เปลื่ยนชื่อ"}</Button>
              </div>
            </div>
            {useMoveTerminology && (
              <p css={tw`text-xs mt-2 text-neutral-400`}>
                <strong css={tw`text-neutral-200`}>ที่อยู่ใหม่:</strong>
