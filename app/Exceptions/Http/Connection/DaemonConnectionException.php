...
class DaemonConnectionException extends DisplayException
{
    ...
    public function __construct(GuzzleException $previous, bool $useStatusCode = true)
    {
        ...
        if (is_null($response)) {
            $message = 'ไม่สามารถเชื่อมต่อกับเครื่องที่กำลังเปิดใช้งานเซิร์ฟเวอร์นี้ได้ กรุณาลองอีกครั้งในภายหลัง';
        } else {
            $message = sprintf('มีข้อผิดพลาดในการสื่อสารกับเครื่องที่กำลังเปิดใช้งานเซิร์ฟเวอร์นี้ ข้อผิดพลาดนี้ได้ถูกบันทึกลงในระบบ กรุณาลองอีกครั้งในภายหลัง (code: %s) (request_id: %s)', $response->getStatusCode(), $this->requestId ?? '<nil>');
        }
        ...
        if ($this->statusCode < 500 && !is_null($response)) {
            ...
            $message = sprintf('เกิดข้อผิดพลาดบนโฮสต์ระยะไกล: %s. (request id: %s)', $body['error'] ?? $message, $this->requestId ?? '<nil>');
        }
